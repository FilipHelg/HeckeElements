#ifndef KL_INVOLUTION_CHECK_SKIP_INCLUDE
...
static void PrintSparseHecke(FILE *out, const FastContext *ctx, const SparseHecke *value, const char *permTable) {
    if (value->supportSize <= 0) {
        fprintf(out, "0");
        return;
    }

    int first = 1;
    for (int i = 0; i < value->supportSize; i++) {
        int idx = value->support[i];
        if (!first) {
            fprintf(out, " + ");
        }
        FprintPoly(out, &value->coeff[idx]);
        fprintf(out, "*C_{%s}", permTable + (size_t)idx * (size_t)(ctx->n + 1));
        first = 0;
    }
}
...
#ifdef KL_INVOLUTION_CHECK_USE_P
    /* Use P-tableau for the bucket key */
    memcpy(key->bytes, P, sizeof(P));
#else
    /* Use Q-tableau for the bucket key */
    memcpy(key->bytes, Q, sizeof(Q));
#endif
    /* Note: do NOT include shape; shape defines two-sided cells, not the bucket key here */
}
...
/* Check if element w is an involution: w * w = identity */
static int IsInvolution(int n, int w) {
    int product = MultiplyIndex(n, w, w);
    return product == 0; /* Identity has index 0 */
}
...
static int CompareBucketProducts(
    ...
    for (int i = 0; i < bucketSize; i++) {
        ...
        SparseClear(product);
#ifdef KL_INVOLUTION_CHECK_MULTIPLY_RIGHT
        MultiplyHeckeSparse(ctx, w, kx, dw, product, tmp1, tmp2, accum, NULL, 0);
#else
        MultiplyHeckeSparse(ctx, x, dw, kx, product, tmp1, tmp2, accum, NULL, 0);
#endif

        fprintf(stderr, "Check w=%d, x=%d\n", w, x);
        fprintf(stderr, "  dw = ");
        PrintSparseHecke(stderr, ctx, dw, permTable);
        fprintf(stderr, "\n  kx = ");
        PrintSparseHecke(stderr, ctx, kx, permTable);
        fprintf(stderr, "\n  product = ");
        PrintSparseHecke(stderr, ctx, product, permTable);
        fprintf(stderr, "\n");
...
static void PrintInvolutionUsage(const char *prog) {
    printf("Usage:\n");
    printf("  %s n w [--dual-bin PATH] [--kl-data PATH] [--out PATH]\n", prog);
    printf("\n");
    printf("Analyzes an involution w (given by lehmer index) in S_n.\n");
        printf("Finds all pairs x,y with the same %s-tableau bucket where dKH_w * kH_x = dKH_w * kH_y (non-zero).\n",
    #ifdef KL_INVOLUTION_CHECK_USE_P
        "P"
    #else
        "Q"
    ...
    printf("Default output: kl_involution_n<n>_w<w>.csv\n");
    printf("Outputs results to CSV and prints summary to terminal.\n");
}

static int ParseInvolutionArgs(
    int argc,
    char *argv[],
    int *nOut,
    int *wOut,
...
int main(int argc, char *argv[]) {
    ...
    if (!ParseInvolutionArgs(argc, argv, &n, &w, &dualPath, &klDataPath, &outputPath)) {
        PrintInvolutionUsage(argv[0]);
        return 1;
    }
    ...
    if (w < 0 || w >= factorialN) {
        fprintf(stderr, "w must be a valid Lehmer index (0 to %d) for S_%d.\n", factorialN - 1, n);
        return 1;
    }

    /* Check if w is an involution */
    if (!IsInvolution(n, w)) {
        char perm[8];
        IndexToPerm(n, w, perm);
        fprintf(stderr, "Error: w=%d is not an involution in S_%d.\n", w, n);
    ...
    snprintf(defaultOut, sizeof(defaultOut), "kl_involution_n%d_w%d.csv", n, w);
    ...
    ctx.n = n;
    ctx.count = factorialN;
    ctx.maxLength = n * (n - 1) / 2;

    fprintf(stderr, "Building context tables...\n");
    ...
    if (n >= 4) {
        fprintf(stderr, "Loading KL nontrivial data from %s...\n", klDataPath);
    ...
    fprintf(stderr, "Loading dual cache from %s...\n", dualPath);
    ...
    if (!dualCache.present[w]) {
        fprintf(stderr, "Error: w=%d is not in the dual KL cache. It may not be an involution or the cache may be incomplete.\n", w);
        DualCacheFree(&dualCache);
        FreeContext(&ctx);
        return 1;
    }
    ...
    int64_t pairCount = 0;
    fprintf(stderr, "Building same-%s-tableau buckets...\n",
#ifdef KL_INVOLUTION_CHECK_USE_P
        "P"
#else
        "Q"
    ...
    if (!BuildSameCellEntries(n, ctx.count, &entries, &pairCount)) {
        fprintf(stderr, "Failed to build same-%s-tableau pair buckets.\n",
#ifdef KL_INVOLUTION_CHECK_USE_P
            "P"
#else
            "Q"
    ...
    if (!SparseInit(&kxTemp, ctx.count) || !SparseInit(&tmp1, ctx.count) ||
        ...
        fprintf(stderr, "Processing same-%s-tableau buckets for w=%d...\n",
    #ifdef KL_INVOLUTION_CHECK_USE_P
        "P",
    #else
        "Q",
    ...
    int bucketsNonEmpty = 0;

    /* Iterate over same-tableau buckets */
    for (int i = 0; i < ctx.count; ) {
        ...
        if (bucketSize > 1) {
            bucketsNonEmpty++;
            if (!CompareBucketProducts(out, &ctx, w, &dualCache.records[w], &entries[i], bucketSize, permTable, n + 1, &accum, &leftCache, &tmp1, &tmp2, &kxTemp, &product, &writtenTripleCount)) {
                fprintf(stderr, "Failed while comparing bucket starting at %d.\n", i);
                SparseFree(&kxTemp);
                SparseFree(&tmp1);
                SparseFree(&tmp2);
                SparseFree(&product);
    ...
    /* Print summary */
    char wperm[8];
    IndexToPerm(n, w, wperm);
    printf("\n========================================\n");
    printf("Involution Analysis Complete\n");
    printf("========================================\n");
    printf("S_%d, w=%d: ", n, w);
    for (int i = 0; i < n; i++) {
        printf("%d ", (int)wperm[i]);
    }
    printf("\n");
    printf("Results written to: %s\n", outputPath);
        printf("Total %s-tableau buckets processed: %d\n",
    #ifdef KL_INVOLUTION_CHECK_USE_P
        "P",
    #else
        "Q",
    #endif
        bucketsProcessed);
    printf("Non-empty buckets (size > 1): %d\n", bucketsNonEmpty);
    printf("Pairs found where dKH_w*kH_x = dKH_w*kH_y (non-zero): %lld\n", (long long)writtenTripleCount);
    printf("========================================\n");
