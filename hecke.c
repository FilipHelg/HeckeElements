...
Laurent_t* MultiplyHecke(int n, Laurent_t H1[], Laurent_t H2[]){
    ...
    for(int i = 0; i < tgamma(n+1); i++){
        ...
        for(int k = 27; k >= 0; k--){
            ...
            if(generator == 0) continue;
            Laurent_t* new_term = (Laurent_t*)calloc((size_t)tgamma(n + 1), sizeof(Laurent_t));
...
Laurent_t* MultiplySimpleHecke2(int n, int s, Laurent_t H[]){
    int count = (int)tgamma(n + 1); // n! element
...
Laurent_t* MultiplyHecke2(int n, Laurent_t H1[], Laurent_t H2[]){
    ...
    for(int i = 0; i < count; i++){
        ...
        // Starta term som H2[i] på rätt index
        Laurent_t* term = calloc(count, sizeof(Laurent_t));
        term[i] = H2[i];

        // Få reducerat uttryck i generatorer för i
        char expression[28];
        int len = ReducedExpression(n, i, expression); // expression[0..len-1]
        ...
        // Multiplicera successivt med varje generator
        for(int k = 0; k < len; k++){
            int s = expression[k];   // 1..n-1
            term = MultiplySimpleHecke2(n, s, term);
        }

        // Lägg till term i produkten
